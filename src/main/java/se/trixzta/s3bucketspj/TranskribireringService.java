...
@Service
public class TranskribireringService implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        BedrockRuntimeClient bedrockRuntimeClient = BedrockRuntimeClient.builder().credentialsProvider(new AwsCredentialsProvider() {
                    @Override
        ...
        try {
            ...
            ConverseResponse response = bedrockRuntimeClient.converse(request -> request.modelId(bedrockModel).messages(message).inferenceConfig(config -> config.maxTokens(512).temperature(0.5F).topP(0.9F)));
    ...
    private void runPolly() {
        ...
        TranscribeClient transcribeClient = TranscribeClient.builder().credentialsProvider(new AwsCredentialsProvider() {
            @Override
            public AwsCredentials resolveCredentials() {
                return AwsBasicCredentials.builder().accessKeyId(accessKey).secretAccessKey(secretKey).build();
            }
    ...
    private static void startTranscriptionJob(TranscribeClient transcribeClient, String transcriptionJobName, String bucketName, String keyName) {
        ...
        StartTranscriptionJobRequest request = StartTranscriptionJobRequest.builder().transcriptionJobName(transcriptionJobName).languageCode(LanguageCode.EN_US) // Ange språk
                .media(Media.builder().mediaFileUri("s3://" + bucketName + "/" + keyName).build()).build();
